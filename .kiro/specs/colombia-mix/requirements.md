...
La pantalla sigue el mismo patrón estructural y de comportamiento que `src/pages/panico-disforico.html`, reutilizando los tokens de diseño (`src/styles/variables.css`) y los componentes de Header y navegación (`src/components/header.html`, `src/components/nav.html`), garantizando coherencia visual y de comportamiento en todo el sitio.
...
- **Pantalla_Colombia_Mix**: Página del sitio web Nous Concepts que presenta el proyecto original "Colombia Mix", compuesta por Header, Etiqueta_Categoría, Título_Página, Sección_Descripción, Sección_Hero, Botón_CTA y Tab_Bar.
...
- **Modo_Claro**: Esquema de colores con fondo crema/beige (`#f5f5f5` o equivalente) y texto dark navy (`#1a1e2d` aprox, `#1f2937` o `#1e3a5f`) utilizado en esta pantalla y en `panico-disforico.html`.
- **Tema_Light**: Configuración `data-theme="light"` aplicada al elemento `<html>` que activa el Modo_Claro en la pantalla.
...
1. THE Pantalla_Colombia_Mix SHALL presentar los elementos en el siguiente orden vertical: Header, Etiqueta_Categoría, Título_Página, Sección_Descripción, Sección_Hero, Botón_CTA, Tab_Bar.
2. THE Pantalla_Colombia_Mix SHALL existir como un archivo HTML independiente en `src/pages/colombia-mix.html`.
3. THE Pantalla_Colombia_Mix SHALL cargar el Header mediante `loadComponent('#header-placeholder', '../components/header.html')` y el Menú_Navegación mediante `loadComponent('#nav-placeholder', '../components/nav.html')`, siguiendo el mismo patrón de inicialización que `src/pages/panico-disforico.html`.
...
7. THE Pantalla_Colombia_Mix SHALL incluir la hoja de estilos `src/styles/variables.css`, `src/styles/main.css` y `src/styles/components.css` en el `<head>`, en ese orden, antes de `src/styles/colombia-mix.css`.
...
2. THE Pantalla_Colombia_Mix SHALL aplicar un fondo claro al elemento `<body>` con el valor de color `#f5f5f5` como valor de respaldo literal (único color literal permitido fuera de `variables.css`), garantizando visibilidad en caso de fallo al cargar la hoja de estilos de variables.
3. THE Pantalla_Colombia_Mix SHALL aplicar color de texto dark navy (`#1f2937` o equivalente dark navy definido en el proyecto) al Título_Página y a los párrafos de la Sección_Descripción.
4. THE Pantalla_Colombia_Mix SHALL utilizar los Tokens_Diseño de `src/styles/variables.css` para tipografía (`--font-heading`, `--font-body`), espaciado (`--spacing-*`) y dimensiones (`--nav-height`), sin redefinir estos valores en `src/styles/colombia-mix.css`.
...
3. THE Etiqueta_Categoría SHALL aplicar un tamaño de fuente menor al del cuerpo de texto, utilizando `var(--font-size-sm, 0.75rem)`, y un espaciado entre letras (`letter-spacing`) de al menos 0.1em para reforzar la apariencia de etiqueta en mayúsculas.
4. THE Etiqueta_Categoría SHALL aplicar un color que contraste visualmente con el fondo claro, usando un tono gris-azulado dark navy coherente con el estilo de `panico-disforico.css` (equivalente a `#6b7280` o definición similar en el tema light).
...
3. THE Título_Página SHALL aplicar color dark navy (equivalente a `#1f2937` o el valor definido en el tema light del proyecto), con contraste mínimo de 4.5:1 sobre el fondo claro.
...
3. THE Sección_Descripción SHALL utilizar el color dark navy/gris-azulado del tema light (equivalente a `#6b7280` o `#1f2937` según jerarquía visual) sobre el fondo claro de la página.
4. WHILE el ancho del Viewport es menor o igual a 768px, THE Sección_Descripción SHALL utilizar el tamaño de fuente `--font-size-base` (1rem) para el texto del párrafo.
5. WHILE el ancho del Viewport es mayor a 768px, THE Sección_Descripción SHALL utilizar el tamaño de fuente `--font-size-lg` (1.25rem) para el texto del párrafo.
...
3. THE Imagen_Hero SHALL renderizarse con `width: 100%` y `height: auto` dentro de su contenedor, manteniendo el aspect-ratio original sin desbordamiento horizontal en cualquier tamaño de Viewport entre 320px y 1920px.
4. THE Imagen_Hero SHALL aplicar `border-radius` de 8px para coherencia con el estilo visual del sitio (igual que en `panico-disforico.css`).
...
6. WHILE la Imagen_Hero se carga, THE Sección_Hero SHALL reservar espacio vertical mediante un contenedor con `aspect-ratio` o dimensiones definidas, para evitar desplazamiento de contenido (Cumulative Layout Shift).
7. IF la Imagen_Hero no puede ser cargada (archivo no encontrado o error de red), THEN THE Pantalla_Colombia_Mix SHALL mostrar el texto alternativo del atributo `alt` dentro del espacio reservado del contenedor, sin colapsar el layout de las secciones adyacentes.
...
2. THE Botón_CTA SHALL aplicar estilos de botón pill (redondeado) con `border-radius` de al menos 24px, fondo dark navy (`#1e3a5f` o equivalente definido en el tema light del proyecto) y texto blanco (`#ffffff`).
...
4. THE Botón_CTA SHALL aplicar padding horizontal de `--spacing-md` (2rem) y padding vertical de `--spacing-xs` (0.5rem) para garantizar un área de toque mínima de 44px de altura en dispositivos móviles.
5. WHEN el Botón_CTA recibe el foco del teclado, THE Botón_CTA SHALL mostrar un indicador de foco visible mediante `outline: 2px solid var(--color-accent)` con `outline-offset: 2px`, cumpliendo contraste mínimo de 3:1 respecto al fondo adyacente.
...
4. THE Tab_Bar SHALL mostrar en el ítem activo "Contenido" el ícono de play (`<svg>` con círculo y triángulo rellenos) con el color dark navy del tema light, y en los ítems inactivos íconos de trazo con color gris (`#9ca3af` o equivalente).
...
8. THE Tab_Bar SHALL incluir en cada ítem `<a>` un atributo `aria-label` descriptivo que indique la acción y el estado: "Ir a Inicio", "Ir a Contenidos (activo)" para el ítem activo, e "Ir a Nosotros".
...
1. THE Pantalla_Colombia_Mix SHALL definir los estilos base (sin media query) aplicando al contenedor `.colombia-mix` un `padding-top` de 96px (suma de `--nav-height` 64px más `--spacing-md` 32px), `padding-inline` de `--spacing-sm` (1rem), y `padding-bottom` de 72px para dejar espacio sobre la Tab_Bar.
...
3. WHILE el ancho del Viewport es mayor a 768px, THE Pantalla_Colombia_Mix SHALL aplicar `padding-inline: var(--spacing-md)` (2rem) al contenedor `.colombia-mix` y escalar el Título_Página a `--font-size-hero` (3.5rem) y el texto de la Sección_Descripción a `--font-size-lg` (1.25rem).
...
1. THE Pantalla_Colombia_Mix SHALL utilizar un elemento `<main>` con `id="main-content"` y atributo `aria-label` cuyo valor identifique la página como presentación del proyecto Colombia Mix (ej. "Colombia Mix — Antología Musical").
...
9. WHEN los componentes Header y Menú_Navegación se cargan dinámicamente mediante JavaScript, THE Pantalla_Colombia_Mix SHALL mantener todos los atributos ARIA (`aria-label`, `aria-expanded`, `aria-controls`) y roles definidos en los componentes fuente, siendo los elementos interactivos inyectados alcanzables mediante teclado sin requerir recarga de la página.
10. THE Tab_Bar SHALL asegurar que el ítem activo "Contenido" sea perceptible mediante lectores de pantalla a través del atributo `aria-label` que incluya el estado "(activo)" o equivalente en español.
...
2. THE Pantalla_Colombia_Mix SHALL incluir en el `<head>` el atributo `lang="es"` en el elemento `<html>`, para indicar correctamente el idioma del contenido a navegadores y tecnologías de asistencia.
...
Las siguientes propiedades de corrección están destinadas a ser verificadas mediante pruebas basadas en propiedades (Property-Based Testing) utilizando el framework de pruebas del proyecto (Vitest).
